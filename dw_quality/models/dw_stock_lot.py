...
class ProductionLot(models.Model):
    ...
    quality_check_qty = fields.Integer(compute='_compute_quality_check_qty')
    quality_alert_qty = fields.Integer(compute='_compute_quality_alert_qty')
    ...
    def _compute_quality_check_qty(self):
        for prod_lot in self:
            ...
            prod_lot.quality_check_qty = self.env['dw.quality.check'].search_count(domain)
    ...
    def action_open_quality_checks(self):
        self.ensure_one()
        action_values = self.env['ir.actions.act_window']._for_xml_id('dw_quality.quality_check_action_production_lot')
    ...
    def _compute_quality_alert_qty(self):
        for prod_lot in self:
            prod_lot.quality_alert_qty = self.env['dw.quality.alert'].search_count([
                ('lot_id', '=', prod_lot.id),
                ('company_id', '=', self.env.company.id)
            ])

    def action_lot_open_quality_alerts(self):
        action = self.env["ir.actions.act_window"]._for_xml_id("dw_quality.quality_alert_action_check")
        action.update({
