...
class StockPicking(models.Model):
    ...
    check_ids = fields.One2many('dw.quality.check', 'picking_id', 'Checks')
    ...
    quality_alert_ids = fields.One2many('dw.quality.alert', 'picking_id', 'Alerts')
    ...
    def _search_quality_check_todo(self, operator, value):
        ...
        pick_ids = self.env['dw.quality.check'].search(domain).picking_id.ids
    ...
    def action_open_quality_check_picking(self):
        action = self.env["ir.actions.actions"]._for_xml_id("dw_quality.quality_check_action_picking")
    ...
    def action_open_on_demand_quality_check(self):
        ...
        return {
            ...
            'res_model': 'dw.quality.check.on.demand',
            'views': [(self.env.ref('dw_quality.quality_check_on_demand_view_form').id, 'form')],
            'target': 'new',
            'context': {
                'default_picking_id': self.id,
