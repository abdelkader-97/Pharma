...
class QualityCheckOnDemand(models.TransientModel):
    _name = 'dw.quality.check.on.demand'
    _description = "Wizard to select on-demand quality check points"

    picking_id = fields.Many2one('stock.picking', string='Picking')
    product_id = fields.Many2one('product.product', string='Product',
                                 domain="[('id', 'in', allowed_product_ids)]")
    quality_point_id = fields.Many2one('dw.quality.point', string='Quality Check Point',
    ...
    allowed_quality_point_ids = fields.One2many('dw.quality.point', compute='_compute_allowed_quality_point_ids')
    ...
    def _compute_allowed_quality_point_ids(self):
        for wizard in self:
            domain = self.env['dw.quality.point']._get_domain(wizard.product_id or wizard.allowed_product_ids, wizard.picking_id.picking_type_id, on_demand=True)
            wizard.allowed_quality_point_ids = self.env['dw.quality.point'].search(domain)
    ...
    def action_confirm(self):
        ...
        self.env['dw.quality.check'].sudo().create(self._get_check_values())
